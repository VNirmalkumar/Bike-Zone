if(new URL(document.location.href).pathname === '/bank-deposit'){
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        bankBalance.innerText = `Rs.₹ ${response.data.bankBalance}`;
        bankDeposits.innerText = `Rs.₹ ${response.data.totalDeposit}`;
    })
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        bankBalance.style.color = parseInt(response.data.bankBalance) < 0 ? 'red' : ''
        bankBalance.innerText = `Rs.₹ ${response.data.bankBalance}`;
        bankWithdrawals.innerText = `Rs.₹ ${response.data.totalWithdraw}`;
    })
