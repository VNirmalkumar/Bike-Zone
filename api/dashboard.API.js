...
const loadDatas = (data) => {
    totalST.innerHTML = 'Rs.₹'+data.sales.totalToday;
    totalBD.innerHTML = 'Rs.₹ '+data.bank.totalDeposit;
    totalBW.innerHTML = 'Rs.₹'+data.bank.totalWithdraw;
    totalBB.innerHTML = 'Rs.₹ '+data.bank.bankBalance;
    totalBB.style.color = parseInt(data.bank.bankBalance) < 0 ? 'red' : '';
    totalE.innerHTML = 'Rs.₹'+data.expenses.totalExpenses;
...
const loadFilteredSales = async () => {
    ...
    if(response.status){
        totalS.innerHTML = `Rs.₹ ${response.data.totalSalesFilter}`;
    }
}
...
const loadDailyFilteredSales = async () => {
    ...
    if(response.status){
        totalST.innerHTML = `Rs.₹ ${response.data.totalDailySalesFilter}`;
    }
}